...
/**
 * The supplier comes from the SupplierField select: an existing supplier id,
 * or "__other__" plus a typed name (resolved/created by the action, so it
 * never duplicates an existing supplier).
 *
 * @returns {{supplierId: string}|{newName: string}|{error: string}}
 */
function parseSupplierChoice(formData) {
  const choice = String(formData.get("supplier_id") ?? "").trim();

  if (choice === "__other__") {
    const newName = String(formData.get("supplier_new") ?? "").replace(/\s+/g, " ").trim();
    if (!newName) return { error: "Enter the new supplier name." };
    if (newName.length > 120) return { error: "Supplier name must be 120 characters or fewer." };
    return { newName };
  }

  if (!choice) return { error: "Select a supplier." };
  if (!z.uuid().safeParse(choice).success) return { error: "Select a valid supplier." };
  return { supplierId: choice };
}

/**
 * Parses a product <form> submission.
 * @returns {{data: object, supplier: {supplierId: string}|{newName: string}}|{fieldErrors: Record<string, string>}}
 */
export function parseProductForm(formData) {
  ...
  const supplier = parseSupplierChoice(formData);

  if (!result.success || supplier.error) {
    ...
    if (supplier.error) errors.supplier = supplier.error;
    return { fieldErrors: errors };
  }

  return { data: result.data, supplier };
}
